  ...
  return (
    <div className='flex flex-wrap gap-1.5'>
      {motifs.map((m) => (
        <span
          key={m.id}
          className='inline-flex items-center gap-1 rounded-element bg-secondary/3 px-2 py-0.5 text-[11px] font-medium text-secondary/60'
        >
          <Link size={10} />
          {m.name}
        </span>
    ...
    <div className='flex flex-wrap gap-1.5'>
      {assignments.map((assignment) => (
        <span
          ...
          className='inline-flex items-center gap-1 rounded-element border border-border-subtle bg-secondary/3 px-2 py-0.5 text-[10px] font-bold text-secondary/50'
        >
          <Link size={9} />
