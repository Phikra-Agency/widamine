...
# MongoDB (required before backend)
./start-mongodb.sh
...
- `~/widamine-backend.log`
- `~/widamine-frontend.log`
- `~/mongodb.log`
...
4. Wait **2–3 minutes** for the backend on `/mnt/c` (Windows drive is slow). Then run `bash scripts/wsl-healthcheck.sh`.

## Env files (created)

- `backend/.env` — from `.env.example`, JWT set for dev
...
cd ~/widamine/backend && npm install --legacy-peer-deps && npx prisma generate
