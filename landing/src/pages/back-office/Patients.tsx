...
import { PencilSimple as Pen, Plus, Trash as Trash2, User, Users, EnvelopeSimple, Phone, MapPin, CalendarBlank, MagnifyingGlass, CaretDown, X, ArrowRight, CalendarDots as CalendarClock } from '@phosphor-icons/react'
    ...
    <div className='flex flex-col gap-2'>
      ...
      <div className={`flex-col gap-2 sm:flex-row sm:flex-wrap ${showExtra ? 'flex' : 'hidden lg:flex'}`}>
        {isPractitioner && (
          <button
            type='button'
            onClick={() => setFilters({ ...filters, practitionerOnly: !filters.practitionerOnly })}
            className={`shrink-0 inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-xs font-semibold border transition-all ${
              filters.practitionerOnly
                ? 'bg-primary/10 border-primary/25 text-primary shadow-[0_1px_2px_rgba(46,144,192,0.1)]'
                : 'bg-white border-black/[0.06] text-secondary/50 hover:text-secondary hover:border-secondary/20'
            }`}
          >
            <Users size={14} weight={filters.practitionerOnly ? 'fill' : 'regular'} />
            {filters.practitionerOnly ? 'Mes patients uniquement' : 'Tous les patients'}
          </button>
        )}
          ...
          >
            ...
            <div className='flex-1 min-h-0 overflow-auto px-4 py-3 space-y-3 sm:px-5 sm:py-4 sm:space-y-4'>
              ...
              <button
                ...
                className='w-full rounded-xl border border-black/[0.06] p-4 space-y-2 text-left hover:border-primary/20 hover:bg-primary/[0.02] transition-all cursor-pointer group'
              >
                <div className='flex items-center justify-between mb-2'>
                  <p className='text-[10px] uppercase tracking-[0.22em] text-secondary/40'>Contact</p>
                  <Pen size={14} className='text-secondary/20 group-hover:text-amber-600 transition-colors' />
                </div>
