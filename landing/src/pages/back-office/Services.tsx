...
function Modal() {
  ...
  return (
    <div className={clsx("fixed inset-0 z-50 flex items-start justify-center pt-8 pb-8 px-4", ["create", "edit"].includes(operation) && modalOpen ? "opacity-100" : "opacity-0 pointer-events-none")}>
        ...
        className={clsx("relative w-full max-w-lg max-h-[calc(100vh-4rem)] overflow-y-auto rounded-2xl border border-black/[0.06] bg-white shadow-bo-elevated transition-all duration-300", ["create", "edit"].includes(operation) && modalOpen ? "opacity-100 scale-100" : "opacity-0 scale-95 pointer-events-none")}
      >
...
function SessionModal({ open, onClose, editing, session, setSession }: { open: boolean; onClose: () => void; editing: boolean; session: { id: number; session: number; duration: number }; setSession: (session: { id: number; session: number; duration: number }) => void }) {
    ...
    <div className={clsx("fixed inset-0 z-[60] flex items-start justify-center pt-8 pb-8 px-4", open ? "opacity-100" : "opacity-0 pointer-events-none")}>
			<div className="absolute inset-0 bo-glass transition-opacity duration-300" onClick={onClose} />
			<form
				onSubmit={(e) => {
					e.preventDefault()
                ...
                className={clsx("relative w-full max-w-sm max-h-[calc(100vh-4rem)] overflow-y-auto rounded-2xl border border-black/[0.06] bg-white shadow-bo-elevated transition-all duration-300", open ? "opacity-100 scale-100" : "opacity-0 scale-95")}
			>
...
<div className="absolute inset-0 bo-glass transition-opacity duration-300" onClick={closeModal} />
                <motion.div onClick={(e) => e.stopPropagation()} initial={{ opacity: 0, y: 16, scale: 0.98 }} animate={operation === "show" && modalOpen ? { opacity: 1, y: 0, scale: 1 } : { opacity: 0, y: 8, scale: 0.98 }} transition={{ duration: 0.3, ease: [0.22, 1, 0.36, 1] }} className={clsx("relative w-full max-w-3xl max-h-[calc(100vh-4rem)] overflow-y-auto rounded-2xl border border-black/[0.06] bg-white shadow-bo-elevated transition-all duration-300", operation === "show" && modalOpen ? "opacity-100" : "opacity-0 pointer-events-none")}> 
					<div className="sticky top-0 z-10 border-b border-black/[0.06] bg-white px-6 py-4">
...
function DeleteModal() {
	...
	return (
    	<div className={clsx("fixed inset-0 z-50 flex items-start justify-center pt-8 pb-8 px-4", operation === "delete" && modalOpen ? "opacity-100" : "opacity-0 pointer-events-none")}> 
			<div className="absolute inset-0 bo-glass transition-opacity duration-300" onClick={closeModal} />
			<div onClick={(e) => e.stopPropagation()} className={clsx("relative w-full max-w-md rounded-2xl border border-black/[0.06] bg-white shadow-bo-elevated transition-all duration-300", operation === "delete" && modalOpen ? "opacity-100" : "opacity-0 pointer-events-none")}>
				...
				<div className="border-t border-black/[0.06] px-6 py-4 flex gap-3 justify-end">
					<button onClick={closeModal} className="px-5 py-2.5 rounded-lg text-sm font-medium text-secondary/60 hover:text-secondary hover:bg-secondary/[0.04] transition-all duration-200">
						Annuler
					</button>
					<button onClick={deleteItem} className="px-5 py-2.5 rounded-lg text-sm font-medium text-white bg-red-500 hover:bg-red-600 shadow-sm shadow-red-500/10 transition-all duration-200">
						Supprimer
					</button>
				</div>
			</div>
