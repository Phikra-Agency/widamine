      ...
      <div className='mx-auto w-full max-w-4xl px-4 relative'>
        ...
        >
          ...
          <div className='absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 hidden items-center gap-7 lg:flex'>
            ...
            <Link
              to='/about'
              className='text-[14px] font-medium transition-colors hover:underline hover:decoration-primary/50 hover:underline-offset-4 hover:decoration-1'
          ...
          <div className='flex items-center gap-3'>
            ...
            <button
              ...
              className='hidden cursor-pointer items-center justify-center rounded-full px-6 py-2.5 text-[13px] font-semibold text-white transition-colors duration-500 border-2 hover:!bg-[#007a9e] hover:!border-[#007a9e] hover:!text-white lg:inline-flex'
          ...
          {isMobileMenuOpen ? (
            ...
            >
              <div className='max-h-[78dvh] space-y-1 overflow-y-auto p-4'>
                <Link 
                  to='/' 
                  className='block cursor-pointer rounded-xl px-3 py-2.5 text-sm font-medium transition-colors hover:!text-primary' 
                ...
                {MEGA_CATEGORIES.map((cat) => (
                  <div key={cat.slug} className='space-y-1 pt-2'>
                    <Link
                      to={`/category/${cat.slug}`}
                      onClick={() => setIsMobileMenuOpen(false)}
                      className='group flex items-center gap-2 rounded-lg px-3 py-1.5 text-sm font-bold normal-case tracking-wide text-primary'
                    >
                      {cat.label}
                      <svg className='h-3.5 w-3.5 transition-transform duration-300 ease-out group-hover:translate-x-0.5 text-primary' fill='none' viewBox='0 0 24 24' stroke='currentColor'>
                        <path strokeLinecap='round' strokeLinejoin='round' strokeWidth={2.5} d='M9 5l7 7-7 7' />
                      </svg>
                    </Link>
                    <div className='space-y-0.5'>
                      {cat.items.map((item) => (
                        <Link
                          ...
                          to={item.href}
                          onClick={() => setIsMobileMenuOpen(false)}
                          className='group flex w-full cursor-pointer items-center rounded-xl px-3 py-2.5 text-sm transition-colors hover:opacity-100'
                          style={{ color: C.secondary }}
                        >
                          <span className='truncate font-medium opacity-85 transition-colors duration-200 group-hover:text-primary group-hover:opacity-100'>{item.label}</span>
                        </Link>
