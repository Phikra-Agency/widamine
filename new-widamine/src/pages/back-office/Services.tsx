...
import { createServicesColumns, SERVICES_EMPTY_ICON } from "./columns/servicesColumns"
...
function ServicesTable() {
	...
	const columnFilters = useMemo<ColumnFiltersState>(
		() => [{ id: 'categoryId', value: categoryFilter }],
		[categoryFilter],
	)
	...
	const table = useDataTable({
		...
		globalFilterFn: (row, _columnId, filterValue) => {
			...
			if (!term) return true
			return row.original.name.includes(term)
		},
	})
    ...
    <DataTable.Root>
      ...
      <DataTable.Mobile>
        <DataTable.MobileList>
          ...
          {isEmpty && <DataTable.Empty icon={SERVICES_EMPTY_ICON} title='Aucun service trouvé' />}
          {!loading &&
            rows.map((row) => {
              ...
              <DataTable.MobileCard key={row.id}>
                ...
                <div className='mt-3 space-y-2 text-xs text-secondary/60'>
                  ...
                  <div className='flex items-center justify-between'>
                    ...
                    <span>
                      {item.category?.name ? (
                        <span className='inline-flex items-center rounded-md border border-violet-100 bg-violet-50 px-2 py-0.5 text-[11px] font-medium text-violet-700'>
                          {item.category.name}
                        </span>
                      ) : (
                        '—'
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  {item.sessions?.length === 0 && (
                    <div className="rounded-xl border border-border bg-background p-4 text-sm text-secondary/40 text-center">
                      Aucune séance trouvée
                    </div>
                  )}
                  {item.sessions?.map((session) => (
                    <div key={session.id} className="rounded-2xl border border-border bg-background p-4 flex items-center gap-3">
